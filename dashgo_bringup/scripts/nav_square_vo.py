...
def quat_to_angle(quat):
    ...
    check_result = rot*PyKDL.Vector(1.0,0.0,0.0)
    print "check vector: ",check_result.x(), check_result.y(), check_result.z() 
    atan2( check_result.x(), check_result.z())
    return atan2( check_result.x(), -check_result.z())
...
class NavSquare():
    def __init__(self):
        ...
        for i in range(4):
            ...
            while ( (abs(angle_error) > abs(angular_tolerance) and abs(distance_error)> abs(10.0*distance_tolerance)) or abs(distance_error) > abs(distance_tolerance) ) and not rospy.is_shutdown():
                ...
                print "command: ", move_cmd.linear.x, move_cmd.angular.z
